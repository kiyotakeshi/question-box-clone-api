...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests()
				...
				// .mvcMatchers("/list").authenticated()
				.mvcMatchers("/list").hasAuthority("SCOPE_read:messages").and().cors().and().oauth2ResourceServer()
				.jwt();
	}
	...
	JwtDecoder jwtDecoder() {
		/*
		 * By default, Spring Security does not validate the "aud" claim of the token, to
		 * ensure that this token is indeed intended for our app. Adding our own validator
		 * is easy to do:
		 */
